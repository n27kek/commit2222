...
class IrisModel:
    ...
    def plot_decision_regions(self):
        ...
        resolution =0.2
        '''
        numpy 모듈의 arange 함수는 반열린 구간 [start,stop]에서 step의 크기만큼 일정하게
        떨어져 있는 숫자들을 array 형태로 반환해 주는 함수
        meshgrid 명령은 사각형 영역을 구성하는 가로측의 점들과 세로축의 점을 나타내는 
        두 벡터를 인수로 받아서 이 사각형 영역을 이루는 조합을 출력한다.
        결과는 그리드 포인트의 x값 만을 표시하는 행렬과
        ...
        plt.contourf(xx1,xx2,Z,alpht=0.3,cmap=cmap)
